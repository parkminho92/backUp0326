<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.ArrayList, com.kh.borrow_room.model.vo.BorrowRoom" %>
<%
...
<title>Insert title here</title>
	...
	.broomList{display:inline-block;}
	.broomList div{
		float:left;
		margin-left:90px;
		padding:5px;
	}
	#lostTitle{width:300px;}
    .broomList:hover{
    	cursor:pointer;
...
<div class="layout">
    ...
    <div class="container">
		...
		<%} else{ %>
			<%for(BorrowRoom b:list){ %>
				<div class="broomList">
					<div><%=b.getBorrowRoomNo() %></div>
					<div id="lostTitle"><%=b.getTitle() %></div>
					<div><%=b.getSecretStatus() %></div>
					<div><%=b.getRegiDate() %></div>
					<div><%=b.getReplyStatus() %></div>
				</div>
			<%} %>
		<%} %>
	...
	<script>
		...
		$(".broomList").click(function(){
			
			var borrowRoomNo = $(this).children().eq(0).text();
